...
class DailyCalendarCarouselState extends State<DailyCalendarCarousel>
  ...
  Widget build(BuildContext context) {
    DateTime _selectedDateTime = DateTime(
        widget.selectedDateTime.year, widget.selectedDateTime.month, widget.selectedDateTime.day);
...
class DayCalendarState extends State<DayCalendar> with TickerProviderStateMixin {
  ...
  initState() {
    ...
    _pageCtrl = PageController(
        initialPage: 1, keepPage: true, viewportFraction: widget.calendarStyle.viewportFraction);
...
//    timeDilation = 5.0;
  ...
  _buildCalendar(DateDict dict, int slideIndex) {
    ...
    DateTime endDate = DateTime(this._pageDates[slideIndex].year, this._pageDates[slideIndex].month,
        ...
        GridView.count(
          ...
          children: List.generate(totalItemCount, (index) {
            DateTime currentDate =
                DateTime(year, month, index + 1 - this._startWeekday[slideIndex]);
            ...
            bool isNextMonthDay =
                index >= (DateTime(year, month + 1, 0).day) + this._startWeekday[slideIndex];
                      ...
                      child: Text(
                        ...
                        style: availableDate
                            ...
                            : fixedTextStyle.copyWith(
                                color: Color.lerp(systemGreyColor, fixedTextStyle.color, 0.5)),
                        maxLines: 1,
                      ),
                    ),
                    Align(
                        alignment: Alignment.bottomCenter,
                        child: _renderMarked(currentDate,
                            isSelected: isSelectedDay, isAvailable: availableDate)),
                  ]),
                ),
              ),
            );
  ...
  Widget _renderMarked(DateTime now, {bool isSelected, bool isAvailable}) {
    ...
    final Icon defaultBirthdayWidget = Icon(Icons.cake,
        color: isSelected || !isAvailable ? Colors.white : systemOrangeColor, size: 10.0);
    final Icon defaultEventWidget = Icon(Icons.event,
        color: isSelected || !isAvailable ? Colors.white : systemTealColor, size: 10.0);
  ...
  void _setPage({int page}) async {
    ...
    if (page == null) {
      ...
      DateTime startDate =
          DateTime(dates[page].year, dates[page].month, 1 - this._startWeekday[page]);

      DateTime endDate =
          DateTime(dates[page].year, dates[page].month, totalItemCount - this._startWeekday[page]);
...
class MonthCalendarState extends State<MonthCalendar> with TickerProviderStateMixin {
                  ...
                  child: Center(
                    child: Text(
                      ...
                      style: availableDate
                          ? textStyle
                          : textStyle.copyWith(
                              color: Color.lerp(systemGreyColor, textStyle.color, 0.5)),
                      maxLines: 1,
                    ),
                  ),
                ),
  ...
  void _handleMonthBoxTapped(BuildContext context, int month, int year) {
    ...
    if (widget.dayMonthAnim.value != 1.0 || widget.dayMonthAnim.status != AnimationStatus.completed)
      return;
