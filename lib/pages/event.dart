...
class EventPage extends StatefulWidget {
  ...
  EventPage({Key key, this.events}) : super(key: key);
      ...
      child: DailyCalendarCarousel(
        ...
        checkMarkedCallback: (DateTime date, PickType type, {EventCategory category}) {
          ...
          if (category == EventCategory.holiday) {
            return widget.events.where((EventModel event) {
                  DateTime startDate = event.startDate;
                  DateTime endDate = event.endDate;

                  if (event.intervalType == IntervalType.once) {
                    if (endDate == null) {
                      return date.compareTo(startDate) == 0;
                    } else {
                      return date.compareTo(startDate) >= 0 && date.compareTo(endDate) <= 0;
                    }
                  } else if (event.intervalType == IntervalType.annual) {
        ...
        getEventCallback: (DateTime date) {
          ...
          events.addAll(widget.events.where((EventModel event) {
            DateTime startDate = event.startDate;
            DateTime endDate = event.endDate;
